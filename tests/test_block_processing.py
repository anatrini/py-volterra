...
class TestBlockProcessing:
    ...
    def test_block_equals_continuous(self, realistic_kernel, test_audio_signal,
        ...
        assert rms_diff < 1e-11, \
            f"Block size {block_size}: RMS difference {rms_diff:.2e} > 1e-11"

    @pytest.mark.parametrize("dtype", [np.float32, np.float64])
    def test_block_processing_different_dtypes(self, realistic_kernel, test_audio_signal,
                                               sample_rate, dtype):
        """Test block processing with different dtypes."""
        audio = test_audio_signal.astype(dtype)
        block_size = 512

        proc_continuous = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        proc_blocks = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        ...
        assert_allclose(output_continuous, output_blocks, rtol=rtol, atol=atol,
    ...
    def test_reset_state_functionality(self, realistic_kernel, test_audio_signal, sample_rate):
        ...
        processor = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        ...
        processor2 = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        out2_fresh = processor2.process_block(block1)

        out2_reset = processor.process_block(block1)

        # Outputs should match
        assert_allclose(out2_reset, out2_fresh, rtol=1e-12, atol=1e-14,
    ...
    def test_consecutive_blocks_continuity(self, realistic_kernel, test_audio_signal, sample_rate):
        ...
        processor_cont = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        output_continuous = processor_cont.process(audio, block_size=len(audio))

        # Should be identical
        assert_allclose(output_blocked, output_continuous, rtol=1e-10, atol=1e-12,
    ...
    def test_variable_block_sizes(self, realistic_kernel, test_audio_signal, sample_rate):
        ...
        # Process continuously
        processor_cont = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        output_continuous = processor_cont.process(audio[:len(output_variable)], block_size=len(audio))

        # Should match
        assert_allclose(output_variable, output_continuous[:len(output_variable)],
    ...
    def test_partial_block_processing(self, realistic_kernel, sample_rate):
        ...
        assert_allclose(output, output_cont, rtol=1e-10, atol=1e-12,
                       err_msg="Partial block processing failed")
    ...
    def test_single_sample_blocks(self, realistic_kernel, sample_rate):
        ...
        assert_allclose(output_single, output_cont, rtol=1e-10, atol=1e-12,
    ...
    def test_very_long_signal(self, realistic_kernel, sample_rate):
        ...
        # Block processing
        processor_blocks = VolterraProcessorFull(realistic_kernel, sample_rate=sample_rate, use_numba=False)
        ...
        assert_allclose(output_blocks, output_cont, rtol=1e-10, atol=1e-12,
...
class TestStateManagement:
    ...
    def test_history_updates_after_block(self):
        ...
        assert_allclose(actual_in_history, expected_in_history, rtol=1e-12, atol=1e-14,
